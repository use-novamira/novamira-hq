...
- Use `novamira-hq hosting ...` for host and platform operations: environments, backups, domains, DNS, cache, PHP, redirects, denied IPs, logs, analytics, SSH/SFTP, provider WP-CLI, plugin and theme operations, provider operation polling, and installing or configuring the Novamira plugin.
...
Always use `--json` for automation, and never put provider credentials, WordPress admin passwords, SFTP passwords, SSL private keys, or other secrets in command argv when an env/stdin/file option exists.
