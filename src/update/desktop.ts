...
const RELEASES = "https://updates.novamira.ai/api/novamira-hq/releases";
...
export class DesktopUpdateChecker {
  ...
  constructor(
    ...
    this.statsPath = join(stateDir, "desktop-update-stats.json");
  ...
  /** Called only for actual requests, under the update lock. No machine fingerprint. */
  private async headers(): Promise<Record<string, string>> {
    const headers: Record<string, string> = { Accept: "application/json" };
    if (!(
      this.options.statsEnabled ??
      !["0", "false"].includes(process.env.NOVAMIRA_HQ_UPDATE_STATS ?? "")
    ))
      return headers;
    try {
      const month = new Date(this.now()).toISOString().slice(0, 7);
      let id: string | undefined;
      if (await this.security.verifyFile(this.statsPath).catch(() => false)) {
        const record = JSON.parse(
      ...
      if (!id) {
        ...
        await atomicWriteFile(
          this.statsPath,
          `${JSON.stringify({ month, id })}\n`,
          this.security,
        );
      }
      headers["X-Novamira-Install"] = id;
      headers["X-Novamira-Version"] = this.options.current;
      headers["X-Novamira-Platform"] =
        this.options.platform ?? process.platform;
      headers["X-Novamira-Arch"] = this.options.arch ?? process.arch;
    } catch {
      // Statistics must never prevent an update check.
    }
    return headers;
  }
