...
/**
 * Constructs the Cloudways client. The access token is the complete
 * credential; Cloudways does not require a separate account identity.
 */
export const createCloudwaysClient: ProviderClientFactory = (
  context: ProviderClientContext,
): ProviderClient => {
  // `length` is a non-secret property of `SecretValue`, so this check does not
  // reveal the access token.
  if (context.secret.length === 0) {
    throw new CliError(
      "credential_missing",
      "The Cloudways Access Token is empty.",
      {
  ...
  return new CloudwaysClient(context);
};

/**
 * The one class in this module. It holds the authenticated `HttpClient` next
 * to the `ProviderClient` methods; every request variant is
