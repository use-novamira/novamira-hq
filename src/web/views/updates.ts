...
export function renderUpdateCard(view: UpdateCardView): Html {
  ...
  return html`<section${idAttr("updates-card")} class="panel updates-panel"${
    ...
    "pill",
    pill.modifier,
  )}>${pill.text}</span></div><p>Check for a newer desktop release. To update, download it, quit Novamira HQ, and replace the application. Your settings and credentials are stored separately.</p></div><dl class="details-list">${detailRow(
    "Current version",
    view.current,
