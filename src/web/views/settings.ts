...
export function renderUninstallHelp(): Html {
  return html`<section class="how-to-card" aria-labelledby="uninstall-title"><h2 id="uninstall-title">Remove Novamira HQ</h2><p>This page explains removal; it does not uninstall anything.</p><section><h3>Desktop app</h3><p>Quit Novamira HQ first. On macOS, move Novamira HQ from Applications to the Trash. On Windows or Linux, remove the downloaded application and its shortcut or launcher. This does not delete your saved settings or credentials.</p></section><section><h3>Package installation</h3><p>If you installed Novamira HQ with npm, remove that installation with:</p><pre class="code-output"><code>npm uninstall -g @novamira/hq</code></pre><p>Remove its launcher separately if you created one. Uninstalling the package does not erase saved settings or credentials.</p></section><section><h3>AI clients and saved data</h3><p>Remove the Novamira HQ connector from each AI client you configured. To remove saved hosting accounts, use Hosting accounts before uninstalling Novamira HQ. This removes local account registrations, not the websites hosted by the provider.</p></section><p>Novamira HQ and Novamira CLI are separate installations. Removing Novamira HQ does not uninstall Novamira CLI, remove its saved profiles or credentials, or uninstall the WordPress plugins on your sites. Keep the CLI if you use Novamira with other agents.</p><section><h3>Remove Novamira CLI (optional)</h3><p>If installed globally with npm, run this in your terminal:</p><pre class="code-output"><code>npm uninstall -g @novamira/cli</code></pre><p>This removes the CLI executable, not the WordPress plugin. It does not guarantee removal of saved profiles or credentials. If installed another way, use that installation method's removal instructions.</p></section><section><h3>Disconnect sites before removing the CLI (optional)</h3><p>Only do this if you also want to disconnect the sites used by your other agents. First list the saved profiles:</p><pre class="code-output"><code>novamira sites list --json</code></pre><p>For each profile you want to disconnect, replace PROFILE_NAME with its exact name and run:</p><pre class="code-output"><code>novamira --site PROFILE_NAME auth logout
