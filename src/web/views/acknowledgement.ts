...
export function renderAcknowledgement(review = false): Html {
  ...
  const action = review
    ...
    : html`<div${ds.signals({ [busy]: false })}><p>We will check the components needed to connect your sites and use them with your AI, and install them if missing. Existing installations are reused. An internet connection may be needed; setup can take a few minutes.</p><div class="button-row"><button type="button" class="button primary"${ds.indicator(busy)}${ds.attrs({ disabled: signal(busy) })}${ds.on("click", post(url("/_dashboard/app/acknowledge"), { include: [] }))}><span${ds.attrs({ hidden: signal(busy) })}>Configure and continue</span><span hidden${ds.attrs({ hidden: not(signal(busy)) })}>Setting up…</span></button></div><p hidden${ds.attrs({ hidden: not(signal(busy)) })} role="status">Checking and preparing the required components. Please keep this window open.</p></div>`;
