...
function requestCard(entry: HistoryEntry): Html {
  return html`<article${idAttr(`request-${entry.id}`)} class="history-request"><header class="history-request-head"><div><h3>${actionLabel(entry.action)}</h3><p>${entry.targetUrl ?? entry.profile}</p></div><div class="history-request-status"><strong>${LABELS[entry.status]}</strong><time>${dateLabel(entry.startedAt)}</time></div></header>${entry.action === "push-environment" ? html`<a class="text-link"${hrefAttr(url("/push", { job: entry.pushJobId ?? entry.id }))}>Open push and check status</a>` : false}<details><summary>Technical details</summary><dl class="details-list">${detail("Request", entry.action)}${detail("Hosting account", entry.profile)}${detail("Started", entry.startedAt)}${detail("Last observed", entry.updatedAt)}${detail("Source", entry.channel)}${detail("Site ID", entry.siteId)}${detail("Source environment ID", entry.sourceEnvironmentId)}${detail("Target environment ID", entry.environmentId)}${detail("Scope", entry.scope)}${detail("Provider operation", entry.operationId)}${detail("Request error", entry.errorCode)}${detail("Workflow", entry.workflowKind)}${detail("Workflow ID", entry.workflowId)}${detail("Workflow outcome", entry.workflowStatus)}${detail("Workflow error", entry.workflowErrorCode)}</dl></details>${verificationAction(entry)}</article>`;
}
...
function historyCards(entries: HistoryView): Html {
  ...
  return html`${[...groups.values()].map((group) => {
    ...
    return html`<article class="history-workflow"><header class="history-request-head"><div><h3>Novamira setup</h3><p>${first.targetUrl ?? first.profile}</p></div><div class="history-request-status"><strong>${status}</strong><time>${dateLabel(first.startedAt)}</time></div></header><details><summary>Setup steps (${group.length})</summary>${group.map(requestCard)}</details></article>`;
  })}`;
}
