...
function actionTable(
  view: ProviderActionsView,
  groups: ProviderActionGroups,
): Html {
  const labels = [
    "Install and set up Novamira",
    ...Object.entries(ACTION_LABELS).map(([capability, label]) => {
      if (view.provider === "instawp" && capability.startsWith("backups."))
        return `${label} (InstaWP Site Versions)`;
      if (view.provider === "cloudways" && capability === "activity.list")
        return "Read staging deployment activity";
      return label;
    }),
  ];
