...
import { post, confirmThen } from "../expr.js";
...
export function renderProPage(view: ProView): Html {
  const params = view.site ? { site: view.site } : {};
  return html`<section class="page flow-page">${pageHeader("Install Novamira Pro", { description: "The Pro plugin for your WordPress site. Novamira HQ does not require a license.", back: { label: "Back to Sites", href: url("/sites") } })}${!view.site ? html`<section class="panel"><div class="form-grid"><p>Select a connected site from Sites to install Novamira Pro.</p></div></section>` : html`<section class="panel"><div class="panel-head"><div><h2>${view.site}</h2><p>WordPress plugin installation</p></div></div><div class="form-grid">${view.message ? html`<h3>Installation complete</h3><p role="status">${view.message}</p>` : view.review ? html`<h3>Review installation</h3><p><strong>${view.review.siteUrl}</strong></p><p>This activates your license for ${view.review.domain} and may use a license slot.</p><p>${view.review.existing ? "The existing Novamira Pro installation will be kept and configured with your saved license." : "Novamira Free is already active. Novamira Pro will be downloaded, installed, activated and configured with your saved license."}</p><div>${actionButton({ label: "Activate license and install Pro", action: post(url("/_dashboard/pro/install", { confirmation: view.review.id, ...params }), { include: [] }), busy: "proForm.busy", pending: "Installing Novamira Pro…", inlinePending: true })}</div>` : view.last4 ? html`<p>Use saved plugin license <strong>••••••••${view.last4}</strong>.</p><p>Check whether Novamira Pro is already installed.</p><div>${actionButton({ label: "Check site", action: post(url("/_dashboard/pro/plan", params), { include: [] }), busy: "proForm.busy", pending: "Checking site…", inlinePending: true })}</div>` : html`<p>Save your Novamira Pro plugin license below to continue with this site.</p>`}</div></section>${!view.last4 && !view.message ? renderPro(view) : false}`}</section>`;
}
