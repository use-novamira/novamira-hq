 ...
 *
 * 1. *No writer, at all.* `@novamira/cli` already abandoned that approach: its
 *    installer delegates to the third-party `skills` CLI
 *    (`npx --yes skills@… add <package-root> --skill novamira --global`), which
 *    knows every agent's on-disk layout and asks the operator which one. HQ
 *    mirrors it, so there is **no `skills install` and no `setup` command**, and
 ...
 * identity — what `npx skills add --skill novamira-hq` registers — while `core`
...
/**
 * The directory `npx skills add --skill novamira-hq --global` installs, and the
...
export interface SkillReadability {
  ...
  /** `skills/novamira-hq/SKILL.md`, the stub `npx skills add` installs. */
