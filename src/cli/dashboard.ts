...
/** Desktop release links only. Source dashboards never check automatically. */
    ...
    const unavailable = (): Promise<never> => {
      return Promise.reject(
        new CliError(
          ...
          "Install a newer desktop release from the download link. Automatic replacement is not implemented.",
        ),
      );
    };
    return {
      desktop: true,
      automatic:
        dependencies.distribution === "desktop" &&
