...
import { updateCheckEnabled, UpdateChecker } from "./update/index.js";
...
export interface RuntimeEnvironment extends PathEnvironment {
  ...
  /** Explicit opt-in for plain-HTTP site provisioning. */
...
export interface MainOverrides {
  ...
  readonly distribution?: "source" | "desktop";
   ...
   * desktop release catalog. It is deliberately separate from the provisioning `fetch`
...
export async function main(
  ...
  try {
    ...
    // All HQ consumers share the private desktop release cache and catalog.
    const createUpdateChecker = (timeoutMs?: number): UpdateChecker =>
      new UpdateChecker(paths.stateDir, locks, security, {
        currentVersion: VERSION,
        statsEnabled: !["0", "false"].includes(
          environment.NOVAMIRA_HQ_UPDATE_STATS ?? "",
        ),
    ...
    const handlers = createCommandHandlers({
      ...
      distribution: overrides.distribution ?? "source",
