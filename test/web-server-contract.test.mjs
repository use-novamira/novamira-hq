...
test("agent CLI setup is hidden and all setup actions are disabled", async () => {
  const calls = [];
  const unexpected = (name) => async () => {
    calls.push(name);
    throw new Error("Disabled setup service was called");
  };
  const { server, cleanup } = await fixture({
    agentSetup: Object.fromEntries(
      ["view", "start", "cancel", "dismiss", "command", "wait"].map((name) => [
        name,
        unexpected(name),
      ]),
    ),
    appAcknowledgement: { accepted: async () => true, accept: async () => {} },
  });
  const patches = [];
  const stream = {
    patchSignals() {},
    patchElements(value) {
      patches.push(renderHtml(value));
    },
    close() {},
  };
  try {
    for (const path of [
      ...
      assert.doesNotMatch(
        ...
        /Connect your agents|Enable command|Install for Claude Code|settings\?tab=agents/,
      );
    }
    const acknowledgement = await server.dispatch(
      request("/_dashboard/app/acknowledge", {
    ...
    await acknowledgement.run(stream);
    assert.doesNotMatch(patches.join(""), /Connect your agents/);
    for (const action of [
      "status",
      "install",
      "repair",
      ...
      await response.run(stream);
      assert.match(patches.join(""), /temporarily unavailable/);
    }
...
test("How to use it explains the complete handoff and links to both entry paths", async () => {
  ...
  try {
    ...
    for (const want of [
      ...
      'href="/sites?new=cli"',
      "AI client you connected through Configure AI",
      "does not contain an AI chat",
      "Configure a different AI agent",
      'href="/configure-ai"',
      "No skill installation is required",
      "Configure MCP",
...
/**
 * The shipped route surface, exactly as the route contract
