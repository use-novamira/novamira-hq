...
test("both installers direct users to MCP without installing a standalone CLI", async () => {
  for (const path of ["install.sh", "install.ps1"]) {
    const source = await readFile(
      new URL(`../${path}`, import.meta.url),
      "utf8",
    );
    assert.ok(source.includes("Configure AI"));
    assert.ok(source.includes("through MCP"));
