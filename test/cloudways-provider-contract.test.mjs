...
/** Obvious fakes. Neither value is, or resembles, a real Cloudways credential. */
const API_KEY = "not-a-real-cloudways-key";
const ACCESS_TOKEN = "not-a-real-cloudways-token";
...
/**
 * A hand-made `ProviderClientContext`, the exact shape `createHostingClientFactory`
 * hands a provider module. `identity` carries the Cloudways account email, which
 * the Go constructor reads from `profile.CompanyID` or `CLOUDWAYS_EMAIL`.
 */
function contextFor(baseUrl, overrides = {}, http = {}) {
  const profile = {
    provider: "cloudways",
    credential: envCredential("CLOUDWAYS_API_KEY"),
    companyId: EMAIL,
  };
  return {
    provider: "cloudways",
    ...
    profile,
    baseUrl,
    secret: new SecretValue(API_KEY, "env", "env:CLOUDWAYS_API_KEY"),
    credentialSource: "env:CLOUDWAYS_API_KEY",
    companyId: EMAIL,
    identity: EMAIL,
    tokenUrl: undefined,
    env: {},
...
function assertAuthRequest(request) {
  assert.equal(request.line, "POST /oauth/access_token");
  assert.equal(
  ...
  for (const expected of [
    ...
    assert.ok(
      request.body.includes(expected),
      `auth body ${request.body} missing ${expected}`,
    );
  }
}
...
/** Nothing HQ hands back may carry the API key or the exchanged token. */
...
test("cloudways validates credentials", async () => {
  await withServer([AUTH_ROUTE], async ({ baseUrl, requests }) => {
    ...
    assert.equal(validation.companyId, EMAIL);
    assert.equal(validation.credential, "env:CLOUDWAYS_API_KEY");

    assert.equal(requests.length, 1);
    assertAuthRequest(requests[0]);
    assertNoSecrets(validation);
  });
});

test("cloudways requires an account email before any network call", async () => {
  await withServer([], async ({ baseUrl, requests }) => {
    assert.throws(() => clientFor(baseUrl, { identity: undefined }), {
      code: "credential_missing",
    });
    assert.throws(() => clientFor(baseUrl, { identity: "   " }), {
      code: "credential_missing",
    });
...
test("cloudways lists sites with one synthetic environment", async () => {
  ...
  await withServer(routes, async ({ baseUrl, requests }) => {
    ...
    assertAuthRequest(requests[0]);
    assert.equal(requests[1].line, "GET /server");
    assertBearer(requests[1]);
  });
});
...
test("cloudways create site honours plain and clone modes", async () => {
  ...
  await withServer(routes, async ({ baseUrl, requests }) => {
    ...
    assert.equal(requests[1].line, "POST /app?application=php&server_id=123");
    ...
    assert.equal(requests[2].line, "POST /app/clone?app_id=456&server_id=123");
...
test("cloudways restart php discovers the versioned fpm service", async () => {
  ...
  await withServer(routes, async ({ baseUrl, requests }) => {
    ...
    assert.equal(requests[1].line, "GET /service?server_id=123&app_id=456");
    assert.equal(
      requests[2].line,
...
test("cloudways operation status normalizes and redacts the raw payload", async () => {
  ...
  await withServer(routes, async ({ baseUrl, requests }) => {
    ...
    assert.equal(requests[1].line, "GET /operation/op-123?id=op-123");
  });
});
...
test("cloudways encodes body values the way the Go client does", async () => {
  ...
  await withServer(routes, async ({ baseUrl, requests }) => {
    ...
    const query = new URL(requests[1].target, "http://127.0.0.1").searchParams;
...
test("cloudways reuses a cached token and never leaks a credential", async () => {
  const diagnostics = [];
  const routes = [
    AUTH_ROUTE,
  ...
  await withServer(routes, async ({ baseUrl, requests }) => {
    ...
    // One token exchange for two authenticated calls.
    assert.equal(requests.length, 3);
    assertAuthRequest(requests[0]);
    assertBearer(requests[1]);
    assertBearer(requests[2]);

    assert.ok(diagnostics.length > 0);
    assertNoSecrets(diagnostics);
    assertNoSecrets(await client.validate());
  });
});
...
test("cloudways rejects a token exchange that returns no access token", async () => {
  const routes = [{ body: JSON.stringify({ expires_in: 3600 }) }];
  await withServer(routes, async ({ baseUrl }) => {
    const client = clientFor(baseUrl);
...
test("cloudways reports a failed token exchange as a credential problem", async () => {
  const routes = [
