...
test("setup SSE failures are notice patches, with no unhandled rejection", async () => {
  ...
  assert.match(patches[0][0], /temporarily unavailable/);
});

test("command-registration failures retain actionable guidance for the dashboard", async () => {
  const service = createAgentSetup({
    command: "unused",
    registration: {
      status: async () => ({ state: "disabled", launcher: "/launcher" }),
      enable: async () => {},
      remove: async () => {},
      repair: async () => {
        throw new Error(
          "Close running HQ commands and MCP agents, then repair command access.",
        );
      },
    },
  });
  await assert.rejects(service.command("repair"), {
    code: "config_error",
    message:
      "Close running HQ commands and MCP agents, then repair command access.",
  });
